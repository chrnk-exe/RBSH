...
if sys.version_info[0] < 3:
    ...
    un_hexlify = lambda s: s.decode('hex')
else:
    import binascii

    hexlify = binascii.hexlify
    un_hexlify = binascii.unhexlify
...
class Keccak:
    def __init__(self, r, c, n, data=None):
        ...
        self.w = self.b // 25
        # 2**l = w
        self.length = int(math.log(self.w, 2))

        self.n_r = 12 + 2 * self.length
        ...
        # A string of hex chars, where each char represents 4 bits.
    ...
    A: 5x5 matrix containing the state, where each entry is a string of hex chars that is 'w' bits long
    ...
    def update(self, arg):
        ...
        # m.update(a); m.update(b) is equivalent to m.update(a+b). arg is a normal bytestring.
    ...
    characters, including null bytes."""
        ...
        # UGLY WARNING
        # Handle bytestring/hex string conversions
        m = _build_message_pair(un_hexlify(self.buffered_data))
        ...
        while output_length > 0:
            ...
            if output_length > 0:
                s = Keccak.KeccakF(s, self.n_r, self.w)

        self.last_digest = un_hexlify(z[:2 * self.n // 8])
        return self.last_digest
...
def _convertStrToTable(string, w, b):
  ...
  string: string of bytes of hex-coded bytes (e.g. '9A2C...')"""
    ...
    # Check that the input parameters are expected
    if w % 8 != 0:
    ...
    for x in range(5):
        for y in range(5):
            ...
            hex_string = string[offset:offset + bits_per_char]
            output[x][y] = _fromHexStringToLane(hex_string)
    return output
...
# print(int.from_bytes(sha3_256(b'1').digest()))
